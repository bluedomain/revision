...
void listCoursesOfSpecificDepartment()
{
	...
	while (temp != NULL)
	{
		if (temp->data.Title.find(id) != string::npos)
		{
			...
			cout << setw(10) << temp->data.CRN << setw(12) << temp->data.Title;
			cout << setw(10) << temp->data.getUnits() << endl;
		}
		temp = temp->next;
	}
}
...
void listCoursesWithEqualSpecificUnit()
{
	...
	while (temp != NULL)
	{
		if (temp->data.Units == un)
		{
			...
			if (time == 1)
			{
				cout << "CRN:      " << "Title:      " << "Units:    " << "Number of students" << endl;
			}
			cout << setw(10) << temp->data.CRN << setw(12) << temp->data.Title << setw(10) << temp->data.Units << setw(18) << (temp->data).Numofstu() << endl;
			count += 1;
		}
		temp = temp->next;
	}
	...
	while (cin >> last)
	{
		while (temp != NULL)
		{
			if (temp->data.LastName == last)
			{
				...
				if (time == 1)
				{
					cout << "ID:       " << "First Name: " << "Middle Initial: ";
					cout << "Last Name:  " << "Enrolled Units: " << "Term:   " << endl;
				}
				cout << setw(10) << temp->data.ID << setw(12) << temp->data.FirstName << setw(16) << temp->data.MiddleInitial << setw(12) << temp->data.LastName;
				cout << setw(16) << temp->data.EnrolledUnits << setw(8) << temp->data.getTerm() << endl;
				found = 1;
			}
			temp = temp->next;
		}
...
void Enroll()
{
	...
	while (cin >> crn)
	{
		...
		if ((over != 1) && (match != 1))
		{
			cout << "There is no such course. Enter 1 for enroll in another course, 2 for quitting enrollment.\n";
			cin >> choice;
			if (choice == 2)
				break;
			else if (choice == 1)
			{
				cout << "Please enter another crn.\n";
...
void readStudentInfo()
{
	...
	while (read >> id >> first >> middle >> last >> units >> term >> c1 >> c2 >> c3 >> c4)
	{
		...
		if (count > 1)
			CS_tail_insert(cshead, CStudentInfo(id, first, middle, last, units, tm, c1, c2, c3, c4));
	}
}
...
void readCourseInfo()
{
	...
	while (read >> crn >> title >> units >> num >> s1 >> s2 >> s3 >> s4 >> s5)
	{
		...
		{
			;
			CC_tail_insert(cchead, CCourse(crn, title, units, num, s1, s2, s3, s4, s5));
		}
	}
}
...
void adddepart()
{
	...
	cin >> depart;
	CD_tail_insert(cdhead, CDepartment(id, depart));
}
...
void addcourse()
{
	...
	CC_tail_insert(cchead, CCourse(crn, title, units, 0, "N/A", "N/A", "N/A", "N/A", "N/A"));
...
void cancelcourse()
{
	...
	if (temp->data.CRN == crn)
	{
		...
		cchead = temp->next;
		delete temp;
		cout << "Successfully cancelled the course.\n";
		return;
	}
	cstemp = cshead;
	while (temp->next != NULL)
	{
		if (temp->next->data.CRN == crn)
		{
			found = 1;
			while (cstemp != NULL)
			{
			...
			CCnode* previous = temp;
			CCnode* del = temp->next;
			...
			for (int i = 0; i < 5; i++)
			{
				while (cstemp != NULL)
				{
					if ((cstemp->data.ID) == temp->data.StudentIDs[i])
					{
						...
						if (time == 1)
						{
							cout << "ID:     " << "First Name     " << "Middle Initial  " << "Last Name   " << "Enrolled Units  " << "Term    " << endl;
						}
						cout << setw(8) << cstemp->data.ID << setw(15) << cstemp->data.FirstName << setw(16) << cstemp->data.MiddleInitial << setw(12) << cstemp->data.LastName;
						cout << setw(16) << cstemp->data.EnrolledUnits << setw(8) << cstemp->data.getTerm() << endl;
					}
					cstemp = cstemp->next;
				}
				cstemp = cshead;
...
{
	CDnode *temp = cdhead;
	cout << "Department ID: " << "Department Name:  " << endl;
	while (temp != NULL)
	{
		cout << setw(15) << temp->data.ID << setw(18) << temp->data.Depart << endl;
		temp = temp->next;
	}
}
